...
export function SentModal({
  ...
  const t = useT();
  const heading = title ?? t.sent.title;
  useEffect(() => {
    if (!open) return;
      ...
      {open && (
        ...
        >
          ...
          <m.div
            ...
            className="relative w-full max-w-[420px] rounded-[var(--radius-xl)] border border-[color:var(--border-strong)] bg-[color:var(--bg-base)] p-8 text-center [box-shadow:var(--shadow-lg),var(--edge-hi)]"
          >
            <button
              onClick={onClose}
              aria-label={t.sent.close}
              autoFocus
